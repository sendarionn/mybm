// core.mjs
var LINK_PATTERN = /\[([^\[\]\n]+)\]/g;
var BOOKMARKLET_PATTERN = /^\s*javascript:/i;
function isBookmarkletUrl(url = "") {
  return BOOKMARKLET_PATTERN.test(url);
}
function bookmarkletSource(url = "") {
...
function resolveLinkTarget(title, bookmarks2) {
  const normalizedTitle = normalize(title);
  const matches = bookmarks2.filter((bookmark) => normalize(bookmark.title) === normalizedTitle);
  if (matches.length === 1) return { type: "bookmark", bookmark: matches[0] };
  return { type: "virtual", title: title.trim(), matches };
}
function targetKey(target) {
  return target.type === "bookmark" ? `bookmark:${target.bookmark.id}` : `virtual:${normalize(target.title)}`;
}
function buildLinkGraph(bookmarks2, metadata2) {
  const relations = /* @__PURE__ */ new Map();
  const add = (key, label) => {
    if (!relations.has(key)) relations.set(key, /* @__PURE__ */ new Set());
    relations.get(key).add(label);
  };
  for (const bookmark of bookmarks2) {
    const sourceKey = `bookmark:${bookmark.id}`;
    for (const label of extractLinks(metadata2[bookmark.id]?.description || "")) {
      const target = resolveLinkTarget(label, bookmarks2);
...
function fieldScore(value, query, base2) {
  const text = normalize(value);
  if (!text) return null;
  if (text === query) return base2;
  if (text.startsWith(query)) return base2 + 10;
  const index = text.indexOf(query);
  if (index < 0) return null;
  const previous = index === 0 ? "" : text[index - 1];
  return /[^\p{L}\p{N}]/u.test(previous) ? base2 + 20 : base2 + 30;
}
function searchBookmarks(bookmarks2, metadata2, rawQuery) {
  const query = normalize(rawQuery);
  if (!query) {
    return [...bookmarks2].sort((a, b) => b.dateAdded - a.dateAdded);
  }
  return bookmarks2.map((bookmark) => {
    const description = metadata2[bookmark.id]?.description || "";
    const scores = [
  ...
  }).filter(Boolean).sort((a, b) => a.score - b.score || b.bookmark.dateAdded - a.bookmark.dateAdded).map(({ bookmark }) => bookmark);
}
...
// editor-model.mjs
function describeEditorLine(text, active) {
  const prefix = text.match(/^[ \t　]+/u)?.[0] || "";
  const links = active ? [] : [...text.matchAll(/\[([^\[\]\n]+)\]/g)].map((match) => ({
    from: match.index,
...
function buildDecorations(view) {
  ...
  for (let number = 1; number <= view.state.doc.lines; number += 1) {
    ...
    if (presentation.bullet) {
      ...
      values.push(Decoration.replace({ widget: new BulletWidget() }).range(line.from, line.from + presentation.prefixLength));
    }
...
function createDescriptionEditor(parent, { onChange, onLink }) {
...
function renderResults() {
  ...
  for (const bookmark of matches) {
    ...
    actions.append(
      ...
      createIconButton("open", isBookmarkletUrl(bookmark.url) ? `${bookmark.title}\u3092\u73FE\u5728\u306E\u30BF\u30D6\u3067\u5B9F\u884C` : `${bookmark.title}\u3092\u65B0\u3057\u3044\u30BF\u30D6\u3067\u958B\u304F`, async () => {
        try {
...
function startEditing(lineIndex = null) {
  ...
  if (lineIndex !== null) {
    ...
    offset = rows.slice(0, lineIndex).reduce((sum, row) => sum + row.length + 1, 0) + (rows[lineIndex]?.length || 0);
  }
